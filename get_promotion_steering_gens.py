...
### automating dst steering ###

def get_dst_steering_vector(layer, src_prompt, dst_prompt=None, top_completion=None, top_completion_logprob=None, top_completion_len=5, top_completion_num_beams=5, optimize_completion_kwargs=None):
    original_args = dict(
        layer=layer, src_prompt=src_prompt
    )
    original_kwargs = dict(
        dst_prompt=dst_prompt, top_completion=top_completion, top_completion_logprob=top_completion_logprob, top_completion_len=top_completion_len, top_completion_num_beams=top_completion_num_beams
    )

    if top_completion is None:
        if dst_prompt is None:
            raise Exception("dst_prompt cannot be None if top_completion is None!")
        top_completion = sample_most_likely_completions_hf(model, tokenizer, dst_prompt, k=top_completion_num_beams, iters=top_completion_len, use_total_probs=True, return_log_probs=True, return_token_probs=False)[0][0]
    if top_completion_logprob is None:
        if dst_prompt is None:
            raise Exception("dst_prompt cannot be None if top_completion_logprob is None!")
        top_completion_logprob = steering_opt.get_completion_logprob_hf(model, dst_prompt, top_completion, tokenizer, return_all_probs=False).item()
    ...
    new_optimize_completion_kwargs = dict(
        ...
        target_loss=-top_completion_logprob, satisfice=False, target_loss_target_iters=1,
    ...
    datapoints = [
        steering_opt.TrainingDatapoint(
            src_prompt,
            dst_completions=[top_completion],
            src_completions=[],
        )
    ]
...
def do_dst_steering_experiment_on_layer(layer, fake_split_step=1, num_examples=1,
    get_dst_steering_vector_kwargs=None, sample_kwargs=None):

    info = dict(
        fake_split_step=fake_split_step,
        num_examples=num_examples,
        get_dst_steering_vector_kwargs=get_dst_steering_vector_kwargs,
        sample_kwargs=sample_kwargs
    )
    
    if get_dst_steering_vector_kwargs is None: get_dst_steering_vector_kwargs = {}
    ...
    for real_prompt_split, fake_prompt_split in zip(training_real_prompts, training_fake_prompts):
        ...
        for dst_prompt, src_prompt in zip(real_prompt_split, fake_prompt_split):
            ...
            vector, steering_vector_info = get_dst_steering_vector(layer, src_prompt, dst_prompt=dst_prompt, **get_dst_steering_vector_kwargs)
            gc.collect()
            torch.cuda.empty_cache()
...
for layer in range(device_idx, len(model.model.layers), len(devices)):
    ...
    start_time = time.time()
    dst_info = do_dst_steering_experiment_on_layer(layer,
        get_dst_steering_vector_kwargs=dict(
            optimize_completion_kwargs=dict(debug=False),
        ),
        sample_kwargs=dict(use_cache=True)
    )
    all_layers[layer] = dst_info
    with open(f'results/just_kidding/dst_generations_all_layers_{device_idx}.pkl', 'wb') as fp:
