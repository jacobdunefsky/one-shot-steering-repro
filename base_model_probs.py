...
num_training_splits = len(generations_all_layers[0]['generations'])
num_vecs = len(generations_all_layers[0]['generations'][0])
split_names = list(generations_all_layers[0]['generations'][0][0]['generations_dict'].keys())
num_splits = len(split_names)
all_layer_vecs_splits = reduce(lambda x, y: x+y, reduce(lambda x, y: x+y, reduce(lambda x, y: x+y,
    [[[[(i,j,k,l) for l in range(num_splits)]
       for k in range(num_vecs)]
      for j in range(num_training_splits)]
     for i in layer_range
    ])))
my_layer_vecs_splits = [x for i, x in enumerate(all_layer_vecs_splits) if i % len(devices) == device_idx ]
...
# process our data!
for layer, training_split_idx, vec_idx, split_idx in tqdm.tqdm(my_layer_vecs_splits):
    split_name = split_names[split_idx]
    split = generations_all_layers[layer]['generations'][training_split_idx][vec_idx]['generations_dict'][split_name]
    cur_results = {
        'layer': layer,
        'training_split_idx': training_split_idx,
        'vec_idx': vec_idx,
